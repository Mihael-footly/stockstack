-- Session lifecycle and reward authority.
--
-- This file is the only way score, units or XP enter the database. Every
-- table those things live in has RLS with no insert or update policy, so a
-- client holding a valid JWT still cannot write a balance; it can only ask
-- these functions to, and they check first.
--
-- NOTE: start_game_session and submit_game_result are both replaced by later
-- migrations in this directory. This file is the first cut of each, kept so
-- the history replays. Read 20260908135315 and 20260908140722 for what is
-- actually running.
...
declare
  ...
  v_recent integer;
begin
  if array_length(p_stock_pool, 1) is null or array_length(p_stock_pool, 1) < 3 then
    raise exception 'a session needs a market of at least three stocks';
  end if;
  ...
  if p_mode = 'daily' then
    ...
    if p_market_date is null then
      raise exception 'a daily session needs its market date';
    end if;
    v_seed := 'daily-' || p_market_date::text;
  else
    v_seed := encode(gen_random_bytes(9), 'hex');
  end if;
  ...
  values (v_user, p_mode, v_seed, p_rules_version, p_stock_pool, p_market_date, coalesce(p_client_info, '{}'::jsonb))
...
-- Submitting a result.
--
-- A run that fails validation is still stored -- silently discarding it would
-- hide an attack and lose an honest player's game to a bug -- but it is
-- marked, earns nothing, and the leaderboard views filter it out.
...
create or replace function public.ensure_daily_market(
  p_date date,
  p_pool text[],
  p_rules_version integer
)
...
begin
  ...
  values (p_date, 'daily-' || p_date::text, p_pool, p_rules_version)
...
revoke all on function public.ensure_daily_market(date, text[], integer) from public;
revoke all on function public.apply_xp(uuid, integer, text, uuid) from public;
...
grant execute on function public.ensure_daily_market(date, text[], integer) to anon, authenticated;
